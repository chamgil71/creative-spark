...
> 💡 **TIP**: 옵시디언 볼트(Vault)와 Quartz의 `content` 폴더를 분리하여 운영 중이라면, GitHub Actions를 활용해 `publish: true`인 파일만 자동으로 선별하여 넘겨주는 파이썬 스크립트 파이프라인을 구축하면 수동 복사의 번거로움을 100% 없앨 수 있습니다.