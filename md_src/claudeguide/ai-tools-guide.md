...
Settings → Integrations → **Connectors** 탭. MCP 기반으로 웹 Claude에서 외부 서비스 데이터를 직접 조회·제어. 현재 13종 제공.
...
- icon: "💬"
  title: "카카오 메시지 연동"
  desc: |
    - PlayMCP(카카오 공식 MCP) 연결 시 KakaoTalk 조회·발송
    - Claude Desktop Settings → MCP 서버에 카카오 MCP 추가
    - "오늘 받은 카카오 메시지 중 업무 내용 요약해줘"
:::
...
- icon: "🤖"
  ...
  tag: "터미널 CLI"
  desc: "npm install -g 후 claude 명령어로 실행. 대용량 컨텍스트(200K 토큰), CLAUDE.md 프로젝트 지침, 메모리 시스템, 훅 자동화가 강점."
...
- icon: "🔬"
  ...
  meta: "무료 사용량 넉넉함|Gemini 2.0 Flash 포함|API 키 발급 무료"
- icon: "📓"
...
- icon: "🖼️"
  title: "GPT Image (DALL-E 4)"
  tag: "이미지 생성"
  desc: "DALL-E 4 기반 고품질 이미지 생성 및 편집. 이미지 위에 텍스트 추가, 특정 부분만 수정(Inpainting) 지원."
...
- icon: "200K"
  title: "Claude 컨텍스트"
  desc: "토큰 기준 (Pro/Max 플랜)"
- icon: "1M"
  title: "Gemini 컨텍스트"
  desc: "Gemini 1.5 Pro 기준"
- icon: "✅"
  title: "Claude 강점"
