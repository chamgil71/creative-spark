...
- API 키나 민감한 비밀번호를 하드코딩하라고 지시하지 않았는가? (
...
## 말하는 대로 코딩되는 마법을 경험하세요

원하는 기능을 자연어로 설명하면 Cursor가 전체 프로젝트를 훑고 코드를 완성합니다.
