...
| **`flow`** | 단계 흐름과 방향성을 시각화 | 단계 카드와 화살표를 이어 프로세스 흐름을 보여줌. | `icon`, `title`, `desc`, `tag`, `color` |
...
| **`compare-before-after`** | Before/After 전후 비교 | 두 상태를 좌우로 나누어 변화 전후를 강조. | `title`, `desc`, `tag`, `color` |
| **`takeaway`** | 핵심 결론 또는 요약 배너 | 문서 마지막 핵심 메시지를 큰 배너로 강조. | `icon`, `title`, `desc`, `color` |
