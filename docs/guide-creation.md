...
* `desc`: 상세 설명 본문. `\n`으로 구분하면 불릿 목록으로 자동 렌더링
* `tag`: 우측 상단 강조 배지 라벨 / 특징 칩 텍스트 (예: `Best`, `추천`)
* `meta`: 부가 속성 또는 파이프(`|`) 기호로 구분된 리스트 상세 항목
* `note`: 카드 하단 배지·메모 영역. `\n`으로 구분하면 ` · `으로 연결
* `color`: 카드 테두리·배지·SVG 불릿 색상 헥스 컬러 (예: `"#D97757"`)
* `bullet`: 아이템별 불릿 기호 오버라이드 (예: `bullet: "✅"`) — `color`보다 우선
...
작성자는 콘텐츠 성격에 맞게 최적의 컴포넌트를 선택해야 합니다. (총 29종 지원)

| 숏코드 표준명 | 권장 핵심 용도 | 비주얼적 특이점 | 주요 매핑 필드 |
| :--- | :--- | :--- | :--- |
| **`plan-grid`** | 플랜 선택 및 요금제 비교 | `featured="true"` 또는 `color` 지정 시 카드 테두리가 밝게 빛나며 살짝 위로 호버링하는 강조 디자인 작렬. | `title`, `tag`, `meta` (기능 목록), `note` (하단 가격 배지), `featured` |
...
| **`part-banner`** | 시리즈 목차의 대단원(부, Part) 헤더 | 좌측에 고유 번호(icon)가 붙고 우측에 부 제목과 설명, 챕터 범위가 나열되며, HSL 브랜드 컬러가 다이내믹하게 동기화. | `icon` (부 번호), `title`, `desc` (태그라인), `tag` (범위), `color` |
...
| **`step-flow`** | 단계 흐름과 방향성을 시각화 | 단계 카드와 화살표를 이어 프로세스 흐름을 보여줌. | `icon`, `title`, `desc`, `tag`, `color` |
| **`level-grid`** | 레벨·난이도·성숙도 구간 표시 | 단계별 수준 차이를 격자 카드로 정리. | `title`, `desc`, `meta`, `note`, `color` |
| **`checkpoint-grid`** | 중간 점검 포인트 목록 | 이모지 기반 체크포인트를 균형 있게 배치. | `icon`, `title`, `desc`, `color` |
| **`compare-diff`** | Before/After 전후 비교 | 두 상태를 좌우로 나누어 변화 전후를 강조. | `title`, `desc`, `tag`, `color` |
| **`takeaway-banner`** | 핵심 결론 또는 요약 배너 | 문서 마지막 핵심 메시지를 큰 배너로 강조. | `icon`, `title`, `desc`, `color` |

### 2) 블록 인자 (`cols=N`, `bullet=X`)

숏코드명 뒤에 공백으로 블록 전체에 적용되는 인자를 지정합니다. 복수 조합 가능합니다.

#### `cols=N` — 가변 컬럼 제어
격자형 숏코드 5종(`icon-grid`, `feature-grid`, `stat-grid`, `compare-grid`, `columns-grid`)에서 컬럼 수를 강제 지정합니다.
...
#### `bullet=X` — 블록 기본 불릿 지정
`desc`의 다중 줄 목록에 사용될 기본 불릿 기호를 블록 단위로 지정합니다. 아이템의 `bullet:` 필드가 있으면 그쪽이 우선합니다.
...
불릿 우선순위: 아이템 `bullet:` 필드 → 블록 `bullet=` 인자 → `DEFAULT_BULLET` (기본 SVG 체크) → `var(--brand)` 색상
...
### 4) `part-banner` (시리즈 대단원 부/Part 헤더) [NEW]
...
* **카드 자체 패딩**: `build-guide.mjs` -> CSS 클래스 `.icon-card, .feature-card, .compare-card, .column-card, .plan-card, .summary-box-card, .badge-item`
...
### 4) `workflow-flow` (가로 순서도) 화살표 및 스텝 조절
...
* **`git-flow` 브랜치 이름 너비**: `build-guide.mjs` -> CSS 클래스 `.branch-row`
