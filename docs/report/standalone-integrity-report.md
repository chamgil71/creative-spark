...
- **기존 문제점:** 독자적인 가로/세로 그리드 레이아웃을 가진 숏코드들(`chapter-list`, `step-flow`, `checkpoint-grid` 등)이 본문 파서의 일괄적 처리로 인해 불필요하게 일반 사각형 카드 클래스(`.card`)에 이중으로 감싸져 그리드가 찌그러지고 외관을 해치던 문제가 있었습니다.
